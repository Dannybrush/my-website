<script>
    ...
    let PII = $state({
      ...
      tagline:
        "Turning complex code into elegant, professional digital experiences",
      about: [
      ...
      projects: [
        {
          ...
          description:
            "Interactive 3D space exploration app built with SvelteKit and Three.js",
        ...
        },
        {
          title: "DataSync Pro",
          description:
            "Real-time database synchronization tool with advanced filtering options",
    ...
    // Handle browser-only features with SSR-safe approach
    let browser = false;
  
    onMount(() => {
      // Set browser flag to true in onMount (client-side only)
      browser = true;
      
      // Add scroll and mousemove event listeners - browser only
      if (typeof window !== 'undefined') {
  ...
  >
    ...
    <div
      class="blob blob-1"
      style:transform={browser ? `translate(${$cursorSpring.x * 0.02}px, ${$cursorSpring.y * 0.02 - parallaxOffset(0.05)}px)` : ''}
    ></div>
    <div
      class="blob blob-2"
      style:transform={browser ? `translateY(${parallaxOffset(-0.1)}px)` : ''}
    ></div>
    <!-- Grid overlay -->
    ...
    >
      ...
      >
        <h1 class="title">
          <span
            class="gradient-text"
            in:fly={{ y: -50, duration: 1000, delay: 300 }}>{PII.name}</span
          >
        </h1>
        ...
        <p class="subtitle" in:fly={{ y: 30, duration: 1000, delay: 900 }}>
          {PII.tagline}
        </p>
    ...
    <section
      id="about"
      class="about-section animated-section"
    ...
    <section
      id="skills"
      class="skills-section animated-section"
    ...
    <section
      id="projects"
      class="projects-section animated-section"
  ...
  <style>
    ...
    padding: 0.5rem 0;
    transition: all 0.3s ease;
    border-bottom: 2px solid rgba(0, 255, 214, 0.3);
  }
  ...
  .socials a:hover .social-icon {
    stroke: #ff4577;
    filter: drop-shadow(0 0 10px rgba(255, 69, 119, 0.6));
  }
  ...
  .social-icon {
    ...
    stroke: #00ffd6;
    transition: all 0.3s ease;
    filter: drop-shadow(0 0 5px rgba(0, 255, 214, 0.4));
  }
